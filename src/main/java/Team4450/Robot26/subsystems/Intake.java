...
public class Intake extends SubsystemBase {
  ...
  public void periodic() {
    ...
    if (this.canPivit) {
      ...
      if (this.pivitCurrentPositionMotorPosition <= this.pivitTargetPositionMotorPosition
          - Constants.INTAKE_PIVIT_TOLERENCE_MOTOR_ROTATIONS) {
        this.intakePivitMotor.set(power);
      } else if (this.pivitCurrentPositionMotorPosition >= this.pivitTargetPositionMotorPosition
          + Constants.INTAKE_PIVIT_TOLERENCE_MOTOR_ROTATIONS) {
        this.intakePivitMotor.set(-power);
      } else {
  ...
  public double getIntakeCurrent() {
    if (canSpin) {
      return intakeLeftMotor.getSupplyCurrent(true).getValueAsDouble()
          + intakeRightMotor.getSupplyCurrent(true).getValueAsDouble();
    } else {
      return -1;
    }
  }
  ...
  public double getIntakeVoltage() {
    if (canSpin) {
      return intakeLeftMotor.getSupplyVoltage(true).getValueAsDouble()
          + intakeRightMotor.getSupplyVoltage(true).getValueAsDouble();
    } else {
      return -1;
    }
  }
